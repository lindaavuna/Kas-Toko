...
export default function HalamanSupplier() {
  ...
  const payables = usePosStore((s) => s.payables);
  const tambahSupplier = usePosStore((s) => s.tambahSupplier);
  const bayarHutang = usePosStore((s) => s.bayarHutangSupplier);
  ...
  function simpanPembayaran() {
    ...
    bayarHutang(bayar.id, n, user);
    toast.success(n === sisa ? `Hutang ${bayar.supplierName} LUNAS. 🎉` : `Bayar ${formatRupiah(n)} ke ${bayar.supplierName} tercatat.`);
    setBayar(null);
    setDigit("");
  }
  ...
  function simpanSupplier() {
    ...
    tambahSupplier(formSupplier.nama.trim(), formSupplier.telepon.trim() || undefined);
    toast.success(`${formSupplier.nama.trim()} ditambahkan ke daftar supplier.`);
    setFormSupplier({ open: false, nama: "", telepon: "" });
  }
