...
export default function HalamanPengaturan() {
  ...
  const cashierAccounts = usePosStore((s) => s.cashierAccounts);
  const tambahKategori = usePosStore((s) => s.tambahKategori);
  const toggleKasirAkun = usePosStore((s) => s.toggleKasirAkun);
  const tambahKasirAkun = usePosStore((s) => s.tambahKasirAkun);
  ...
  function tambahKategoriKlik() {
    if (!kategoriBaru.trim()) return;
    tambahKategori(kategoriBaru.trim());
    toast.success(`Kategori "${kategoriBaru.trim()}" ditambahkan.`);
    setKategoriBaru("");
  }

  function simpanKasir() {
    const hasil = tambahKasirAkun(kasirBaru.nama.trim(), kasirBaru.email.trim(), kasirBaru.pin);
    toast[hasil.ok ? "success" : "error"](hasil.pesan);
    if (hasil.ok) setKasirBaru({ open: false, nama: "", email: "", pin: "" });
  }
        ...
        <TabsContent value="toko" className="space-y-4 pt-2">
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button size="lg" onClick={() => toast.success("Profil toko tersimpan.")}>
            ...
            <CardContent>
              {MODE === "saas" ? (
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <Badge variant="success">Masa Uji Coba Aktif</Badge>
                    <p className="mt-2 text-sm text-muted-foreground">
                      Sewa berikutnya: 09 Oktober 2026 — Rp 50.000/bulan. Data tersimpan aman di cloud.
                    </p>
                  </div>
              ...
              {cashierAccounts.map((a) => (
                <div key={a.id} className="flex items-center gap-3 rounded-lg border p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{a.email} • PIN: {a.pin.replace(/\d/g, "•")}</p>
                  </div>
                  <Badge variant={a.isActive ? "success" : "secondary"}>{a.isActive ? "Aktif" : "Nonaktif"}</Badge>
                  <Button
                    size="sm"
                    variant={a.isActive ? "outline" : "success"}
                    className="h-9"
                    onClick={() => {
                      toggleKasirAkun(a.id);
                      toast.info(`${a.name} ${a.isActive ? "dinonaktifkan" : "diaktifkan lagi"}.`);
                    }}
                  >
            ...
            <CardContent className="space-y-2">
              ...
              {metodeAktif.transfer && (
                <div className="rounded-lg bg-muted/50 p-3 text-sm">
                  ...
                  <p className="font-medium font-money">{TOKO.rekening}</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="ai" className="pt-2">
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button size="lg" onClick={() => toast.success(ai.apiKey || MODE === "self-hosted" ? "Konfigurasi AI tersimpan. Hermes siap diajak ngobrol!" : "Tersimpan — memakai penyedia gratis tanpa API key.")}>
