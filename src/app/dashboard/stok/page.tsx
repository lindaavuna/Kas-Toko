...
export default function HalamanStok() {
  ...
  const catatMasuk = usePosStore((s) => s.catatBarangMasuk);
  const catatKeluar = usePosStore((s) => s.catatBarangKeluar);
  ...
  function simpanMasuk() {
    if (!dialogMasuk || !user) return;
    const n = Number(jumlah || 0);
    if (n <= 0) {
      toast.error("Isi jumlah barang yang masuk.");
      return;
    }
    catatMasuk(dialogMasuk.id, n, catatan, user);
    toast.success(`${n} ${dialogMasuk.unit} ${dialogMasuk.name} masuk. Stok sekarang ${dialogMasuk.stockQty + n}.`);
    setDialogMasuk(null);
  }

  function simpanKeluar() {
    if (!dialogKeluar || !user) return;
    const n = Number(jumlah || 0);
    const hasil = catatKeluar(dialogKeluar.id, n, alasan, catatan, user);
    toast[hasil.ok ? "success" : "error"](hasil.pesan);
    if (hasil.ok) setDialogKeluar(null);
  }
